...
%                      CURRENTLY ASSUMED TO BE SYNCHRONIZED - ALL CAMERAS START RECORDING AT THE SAME TIME.  
...
function [walking_merged, sample_times] = Kinects_Merge(walkings, inferred_vector, timestamps)
...
for kinect_index = 1:length(walkings_sync)
	...
	for time_index = 1:size(walkings_sync{kinect_index}, 1)
		...
		if TooManyInferred || HeadWrong || TwoFootsWrong
			...
			%sample_times_temp{kinect_index}(time_index) = -1 ; 
			%walkings_sync{kinect_index}(time_index,:,:) = -1 ; 
			%timestamps_sync{kinect_index}(time_index) = -1 ; 
		end
	end
	
	%indices_to_delete = find(sample_times_temp{kinect_index} == -1) ; 
